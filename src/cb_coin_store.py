...
class CBCoinStore:
    ...
    async def create(cls, wrapper: DBWrapper2):
        ...
        async with self.db_wrapper.writer_maybe_transaction() as conn:
            ...
            await conn.execute("CREATE INDEX IF NOT EXISTS state on coin_record(state)")
    ...
    async def get_multiple_coin_records(self, coin_names: List[bytes32]) -> List[CBCoinRecord]:
        """Return CBBCoinRecord(s) that have a coin name in the specified list"""
        if len(coin_names) == 0:
            return []
    ...
    async def add_coin_record(self, record: CBCoinRecord, name: Optional[bytes32] = None) -> None:
        if name is None:
            name = record.name()
    ...
    def coin_record_from_row(self, row: sqlite3.Row) -> CBCoinRecord:
    ...
    async def get_coin_record(self, coin_name: bytes32) -> Optional[CBCoinRecord]:
    ...
    async def get_unspent_coins_for_wallet(self, wallet_id: int) -> Set[CBCoinRecord]:
    ...
    async def get_all_unspent_coins(self) -> Set[CBCoinRecord]:
        """Returns set of CoinRecords that have not been spent yet for a wallet."""
        async with self.db_wrapper.reader_no_transaction() as conn:
    ...
    async def get_coin_records_by_puzzle_hash(self, puzzle_hash: bytes32) -> List[CBCoinRecord]:
    ...
    async def get_coin_records_by_parent_id(self, parent_coin_info: bytes32) -> List[CBCoinRecord]:
